...
export interface ImageValidationFailure {
  ok: false;
  errorCode:
    | typeof ErrorCode.INVALID_MIME_TYPE
    | typeof ErrorCode.FILE_TOO_LARGE
    | typeof ErrorCode.CORRUPTED_IMAGE
    | typeof ErrorCode.UNSUPPORTED_IMAGE;
  message: string;
}
...
function validateImageStructure(buffer: Buffer, mimeType: AllowedImageMimeType): boolean {
  switch (mimeType) {
    case "image/png":
