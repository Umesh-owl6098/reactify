...
import { ErrorCode, ImageUploadResponseSchema, } from "@reactify/shared";
...
export async function registerImageRoutes(app, env, storage) {
    app.post("/api/v1/images", async (request, reply) => {
        const file = await request.file();
        if (!file) {
            return sendError(reply, request, 422, ErrorCode.UNSUPPORTED_IMAGE, 'Missing "image" file in multipart form data.');
        }
        if (file.fieldname !== "image") {
            return sendError(reply, request, 422, ErrorCode.UNSUPPORTED_IMAGE, 'Expected multipart field name "image".');
        }
        const buffer = await file.toBuffer();
        const validation = validateImageBuffer(buffer, env.IMAGE_MAX_BYTES);
        if (!validation.ok) {
            const statusCode = validation.errorCode === ErrorCode.FILE_TOO_LARGE ||
        ...
        const response = ImageUploadResponseSchema.parse({
